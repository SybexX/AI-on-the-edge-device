...
void CCamera::CameraDeepSleep(bool enable)
{
    if (CCstatus.CameraDeepSleepEnable != enable)
    {
        ...
        {
            // OV2640 (Normal mode >>> Standby mode = OK), (Standby mode >>> Normal mode = n.OK, must be completely disconnected from the power)
...
esp_err_t CCamera::CaptureToHTTP(httpd_req_t *req, int delay)
{
    if (Camera.getCameraInitSuccessful())
    {
        ...
        LEDOnOff(false); // Status-LED off
        result = httpd_resp_set_type(req, "image/jpeg");

        if (result == ESP_OK)
        {
            result = httpd_resp_set_hdr(req, "Content-Disposition", "inline; filename=raw.jpg");
        }
...
esp_err_t CCamera::CaptureToStream(httpd_req_t *req, bool FlashlightOn)
{
    if (Camera.getCameraInitSuccessful())
    {
        ...
        while (1)
        {
            ...
            if (result == ESP_OK)
            {
                result = httpd_resp_send_chunk(req, (const char *)fb->buf, fb_len);
            }
