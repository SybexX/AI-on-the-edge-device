...
esp_err_t CCamera::setSensorDatenFromCCstatus(void)
{
    ...
    if (s != NULL) {
        ...
        // s->set_special_effect(s, CCstatus.ImageSpecialEffect); // 0 to 6 (0 - No Effect, 1 - Negative, 2 - Grayscale, 3 - Red Tint, 4 -
...
esp_err_t CCamera::CaptureToBasisImage(CImageBasis *_Image, int delay)
{
    ...
    if (!fb) {
        ...
        LogFile.WriteToFile(ESP_LOG_ERROR, TAG,
                            "is not working anymore (CaptureToBasisImage) - most probably caused "
...
void CCamera::LightOnOff(bool status)
{
    ...
    if ((gpioHandler != NULL) && (gpioHandler->isEnabled())) {
        ...
        if (FLASH_MODE == GPIO_PIN_MODE_FLASH_PWM) {
            if (status) {
        ...
        else if (FLASH_MODE == GPIO_PIN_MODE_OUTPUT) {
            // Init the GPIO
