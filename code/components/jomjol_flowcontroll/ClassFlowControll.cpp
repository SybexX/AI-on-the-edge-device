...
void ClassFlowControll::InitFlow(std::string config)
{
    ...
    while ((line.size() > 0) && !(feof(pFile)))
    {
        ...
        else
        {
            ...
            if (fgets(data_temp, 1024, pFile) && !feof(pFile))
            {
                ESP_LOGD(TAG, "Read: %s", data_temp);
                line = std::string(data_temp);
            }
        }
    }
...
void ClassFlowControll::doFlowTakeImageOnly(string time)
{
    for (int i = 0; i < FlowControll.size(); ++i)
    {
        ...
        {
            std::string time_temp = getCurrentTimeString("%H:%M:%S");
            aktstatus = TranslateAktstatus(FlowControll[i]->name());
            aktstatusWithTime = aktstatus + " (" + time_temp + ")";
...
bool ClassFlowControll::doFlow(string time)
{
    ...
    for (int i = 0; i < FlowControll.size(); ++i)
    {
        time_temp = getCurrentTimeString("%H:%M:%S");
        aktstatus = TranslateAktstatus(FlowControll[i]->name());
        aktstatusWithTime = aktstatus + " (" + time_temp + ")";
...
#ifdef DEBUG_DETAIL_ON
        string data_temp = "FlowControll.doFlow - " + FlowControll[i]->name();
        LogFile.WriteHeapInfo(data_temp);
#endif
    ...
    else if (_fn == "alg_roi.jpg")
    {
        ...
        else if (aktstatus.find("Take Image") != -1)
        {
            ...
            else
            {
                ...
                {
                    std::string filename = "/sdcard/html/Flowstate_at_work.jpg";
                    ret = send_file(req, filename);
        ...
        else
        {
            if (flowalignment && flowalignment->AlgROI)
            {
                ...
                ret = httpd_resp_send(req, (const char *)flowalignment->AlgROI->data, flowalignment->AlgROI->size);
            }
            else
            {
                LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "ClassFlowControll::GetJPGStream: alg_roi.jpg cannot be served -> alg.jpg is going to be served!");
