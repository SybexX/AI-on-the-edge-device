    ...
    else if (_fn == "alg_roi.jpg")
    {
        ...
        else if (aktstatus.find("Take Image") != -1)
        {
            if (flowalignment && flowalignment->AlgROI)
            {
                ...
                {
                    LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "File /sdcard/html/Flowstate_take_image.jpg (" + std::to_string(flowalignment->AlgROI->size) + ") > allocated buffer (" + std::to_string(MAX_JPG_SIZE) + ")");
                    fclose(file);
                    return ESP_FAIL;
                }
