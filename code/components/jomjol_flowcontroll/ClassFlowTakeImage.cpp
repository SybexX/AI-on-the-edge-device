...
void ClassFlowTakeImage::takePictureWithFlash(int flash_duration)
{
    ...
    {
        rawImage->width = CFstatus.CamConfig.ImageWidth;
        rawImage->height = CFstatus.CamConfig.ImageHeight;
    }
    else
    {
    ...
    if (CCstatus.CameraTempImage && CFstatus.SaveAllFiles)
    {
...
{
    Camera.getSensorConfig(&CCstatus.CamConfig); // Kamera >>> CCstatus
    ...
    while (this->getNextLine(pfile, &aktparamgraph) && !this->isNewParagraph(aktparamgraph))
    {
        ...
        else if ((toUpper(splitted[0]) == "WAITBEFORETAKINGPICTURE") && (splitted.size() > 1))
        {
            if (isStringNumeric(splitted[1]))
            {
                ...
                {
                    CCstatus.CamConfig.WaitBeforePicture = _WaitBeforePicture;
                }
                else
                {
        ...
        else if ((toUpper(splitted[0]) == "CAMQUALITY") && (splitted.size() > 1))
        {
            if (isStringNumeric(splitted[1]))
            {
                ...
                CCstatus.CamConfig.ImageQuality = clipInt(_ImageQuality, 63, 10);
            }
        }
        ...
        else if ((toUpper(splitted[0]) == "CAMZOOMOFFSETX") && (splitted.size() > 1))
        {
            if (isStringNumeric(splitted[1]))
            {
                ...
                else if (CCstatus.CamConfig.CamSensor_id == OV3660_PID)
                {
                    CCstatus.CamConfig.ImageZoomOffsetX = clipInt(_ImageZoomOffsetX, 704, -704);
                }
                else if (CCstatus.CamConfig.CamSensor_id == OV5640_PID)
                {
        ...
        else if ((toUpper(splitted[0]) == "LEDINTENSITY") && (splitted.size() > 1))
        {
            if (isStringNumeric(splitted[1]))
            {
                ...
                CCstatus.CamConfig.ImageLedIntensity = Camera.SetLEDIntensity(ledintensity);
            }
        }
    ...
    Camera.setSensorConfig(&CCstatus.CamConfig); // CCstatus >>> Kamera
    Camera.setCCstatusToCFstatus();
    Camera.SetQualityZoomSize(&CCstatus.CamConfig);
...
bool ClassFlowTakeImage::doFlow(string zwtime)
{
    ...
    int flash_duration = 2000;
    if (CCstatus.CameraTempImage)
    {
        flash_duration = (int)(CFstatus.CamConfig.WaitBeforePicture * 1000);
    }
    else
    {
        flash_duration = (int)(CCstatus.CamConfig.WaitBeforePicture * 1000);
    }
