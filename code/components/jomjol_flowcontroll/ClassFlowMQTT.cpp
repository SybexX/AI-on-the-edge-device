...
bool ClassFlowMQTT::ReadParameter(FILE *pfile, string &aktparamgraph)
{
    ...
    while (this->getNextLine(pfile, &aktparamgraph) && !this->isNewParagraph(aktparamgraph))
    {
        ...
        if (splitted.size() > 1)
        {
            ...
            else if (toUpper(_param) == "RETAINMESSAGES")
            {
                SetRetainFlag = alphanumericToBoolean(splitted[1]);
                setMqtt_Server_Retain(SetRetainFlag);
            }
            else if (toUpper(_param) == "HOMEASSISTANTDISCOVERY")
            {
            ...
            else if (toUpper(_param) == "METERTYPE")
            {
            ...
            else if (toUpper(_param) == "CLIENTID")
            {
                this->clientname = splitted[1];
            }
            else if ((toUpper(_param) == "TOPIC") || (toUpper(splitted[0]) == "MAINTOPIC"))
            {
                maintopic = splitted[1];
            }
            else if (toUpper(_param) == "DOMOTICZTOPICIN")
            {
                this->domoticzintopic = splitted[1];
            }
            else if (toUpper(_param) == "DOMOTICZIDX")
            {
