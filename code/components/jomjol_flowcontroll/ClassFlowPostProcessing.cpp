...
bool ClassFlowPostProcessing::ReadParameter(FILE *pfile, string &aktparamgraph)
{
    ...
    while (this->getNextLine(pfile, &aktparamgraph) && !this->isNewParagraph(aktparamgraph))
    {
        ...
        if (splitted.size() > 1)
        {
            if (toUpper(_param) == "EXTENDEDRESOLUTION")
            {
                handleDecimalExtendedResolution(splitted[0], splitted[1]);
            }
            else if (toUpper(_param) == "DECIMALSHIFT")
            {
                handleDecimalSeparator(splitted[0], splitted[1]);
            }
            else if (toUpper(_param) == "ANALOGTODIGITTRANSITIONSTART")
            {
...
bool ClassFlowPostProcessing::doFlow(std::string zwtime)
{
    ...
    for (int j = 0; j < NUMBERS.size(); ++j)
    {
        ...
        if (!flowctrl.AlignmentOk)
        {
            ...
            NUMBERS[j]->ErrorMessageText = NUMBERS[j]->ErrorMessageText + "Alignment failed - Read: " + RundeOutput(NUMBERS[j]->Value, NUMBERS[j]->Nachkomma) + " - Raw: " + NUMBERS[j]->ReturnRawValue + " - Pre: " + RundeOutput(NUMBERS[j]->PreValue, NUMBERS[j]->Nachkomma) + " - Rate: " + NUMBERS[j]->ReturnRateValue;
