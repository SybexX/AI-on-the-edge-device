...
void InfluxDB::InfluxDBPublish(std::string _measurement, std::string _key, std::string _content, long int _timeUTC)
{
    ...
    case INFLUXDB_V1:
        ...
        {
            LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "Failed to publish data: " + std::string(esp_err_to_name(ret)));
        }
        break;
    ...
    case INFLUXDB_V2:
        ...
        esp_http_client_set_method(httpClient, HTTP_METHOD_POST);
        esp_http_client_set_header(httpClient, "Content-Type", "text/plain");
        std::string data_temp = "Token " + token;
        esp_http_client_set_header(httpClient, "Authorization", data_temp.c_str());
        esp_http_client_set_post_field(httpClient, payload.c_str(), payload.length());
        ret = ESP_ERROR_CHECK_WITHOUT_ABORT(esp_http_client_perform(httpClient));
        if (ret == ESP_OK)
        {
            LogFile.WriteToFile(ESP_LOG_INFO, TAG, "Data published successfully: " + payload);
        }
        ...
        {
            LogFile.WriteToFile(ESP_LOG_INFO, TAG, "Failed to publish data: " + std::string(esp_err_to_name(ret)));
        }
        break;
    }
}
