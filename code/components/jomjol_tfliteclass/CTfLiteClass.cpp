...
int CTfLiteClass::GetOutClassification(int _von, int _bis)
{
  ...
  float max_temp = output2->data.f[_von];
  int class_temp = _von;
  for (int i = _von + 1; i <= _bis; ++i)
  {
    float data_temp = output2->data.f[i];
...
{
#ifdef DEBUG_DETAIL_ON
  float *input_temp = this->input;
  int test = sizeof(input_temp);
