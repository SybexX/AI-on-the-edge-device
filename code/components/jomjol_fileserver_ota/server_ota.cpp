...
static esp_err_t ota_update_task(std::string fn);
...
void task_do_update_zip(void *pvParameter)
{
    ...
    if (filetype == "ZIP")
    {
        std::string outHtml = "/sdcard/html";
        std::string outHtmlTmp = "/sdcard/html_tmp";
        std::string outHtmlOld = "/sdcard/html_old";
        std::string outbin = "/sdcard/firmware";
        ...
        std::string retfirmware = unzip_ota_file(_file_name_update, outHtmlTmp + "/", outHtml + "/", outbin + "/", "/sdcard/", initial_setup);
...
void checkUpdate(void)
{
    ...
    _file_name_update = std::string(data_temp);
...
static esp_err_t ota_update_task(std::string fn)
{
    ...
    FILE *pFile = fopen(fn.c_str(), "rb"); // previously only "r
    if (pFile == NULL)
    {
        // File does not exist
        return ESP_FAIL;
    ...
    char *chunk[SERVER_OTA_SCRATCH_BUFSIZE + 1] = {0};
    int chunksize = 0;

    do
    {
        chunksize = fread(chunk, 1, SERVER_OTA_SCRATCH_BUFSIZE, pFile);
        ...
        if (image_header_was_checked == false)
        {
            ...
            if (chunksize > sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t) + sizeof(esp_app_desc_t))
            {
                ...
                memcpy(&new_app_info, &chunk[sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t)], sizeof(esp_app_desc_t));
                ...
                if (ret != ESP_OK)
                {
                    LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "esp_ota_begin failed (" + std::string(esp_err_to_name(ret)) + ")");
                    fclose(pFile);
                    return ret;
                }
        ...
        ret = esp_ota_write(update_handle, (const void *)chunk, chunksize);
        if (ret != ESP_OK)
        {
            fclose(pFile);
            return ret;
    ...
    if (ret != ESP_OK)
    {
        ...
        LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "esp_ota_end failed (" + std::string(esp_err_to_name(ret)) + ")!");
        return ret;
    }
    ...
    if (ret != ESP_OK)
    {
        LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "esp_ota_set_boot_partition failed (" + std::string(esp_err_to_name(ret)) + ")!");
    }
...
{
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "ota_update_handler");
    char _query[200];
    char _filename[100];
    char _valuechar[30];
    ...
    if (_task.compare("emptyfirmwaredir") == 0)
    {
        ...
        std::string directory_temp = "firmware directory deleted - v2\n";
        ESP_LOGD(TAG, "%s", directory_temp.c_str());
        printf("Ausgabe: %s\n", directory_temp.c_str());

        httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
        httpd_resp_send(req, directory_temp.c_str(), strlen(directory_temp.c_str()));
    ...
    if (_task.compare("update") == 0)
    {
        ...
        {
            std::string data_temp = "Update failed - no file specified (zip, bin, tfl, tlite)";
            httpd_resp_sendstr_chunk(req, data_temp.c_str());
        ...
        std::string data_temp = "Update failed - no valid file specified (zip, bin, tfl, tlite)!";
        httpd_resp_sendstr_chunk(req, data_temp.c_str());
        httpd_resp_sendstr_chunk(req, NULL);
        return ESP_OK;
    }
    ...
    if (_task.compare("unziphtml") == 0)
    {
        ...
        unzip_file(in, out + "/");
        std::string data_temp = "Web Interface Update Successfull!\nNo reboot necessary";
        httpd_resp_send(req, data_temp.c_str(), strlen(data_temp.c_str()));
        httpd_resp_sendstr_chunk(req, NULL);
        return ESP_OK;
    }
    ...
    if (_file_del)
    {
        ...
        /* Respond with an empty chunk to signal HTTP response completion */
        std::string data_temp = "file deleted\n";
        ESP_LOGD(TAG, "%s", data_temp.c_str());
        httpd_resp_send(req, data_temp.c_str(), strlen(data_temp.c_str()));
        httpd_resp_send_chunk(req, NULL, 0);
        return ESP_OK;
    }

    std::string data_temp = "ota without parameter - should not be the case!";
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    httpd_resp_send(req, data_temp.c_str(), strlen(data_temp.c_str()));
    httpd_resp_send_chunk(req, NULL, 0);
...
void reboot_task(void *DeleteMainFlow)
{
    ...
    if ((network_config.connection_type == NETWORK_CONNECTION_WIFI_AP_SETUP) || (network_config.connection_type == NETWORK_CONNECTION_WIFI_AP))
    {
        wifi_deinit_ap();
    }
...
void doReboot(void)
{
    ...
    BaseType_t xReturned = xTaskCreate(&reboot_task, "reboot_task", configMINIMAL_STACK_SIZE * 4, (void *)true, 10, NULL);
    if (xReturned != pdPASS)
    {
        LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "reboot_task not created -> force reboot without killing flow");
        reboot_task((void *)false);
    }
...
{
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "reboot_handler");
...
esp_err_t register_server_ota_uri(httpd_handle_t my_server)
{
    ...
    httpd_uri_t ota = {
        .uri = "/ota",
        .method = HTTP_GET,
        .handler = APPLY_BASIC_AUTH_FILTER(ota_update_handler),
        .user_ctx = rest_context // Pass server data as context
    };
    ret |= httpd_register_uri_handler(my_server, &ota);

    httpd_uri_t reboot = {
        .uri = "/reboot",
        .method = HTTP_GET,
        .handler = APPLY_BASIC_AUTH_FILTER(ota_reboot_handler),
        .user_ctx = rest_context // Pass server data as context
    };
    ret |= httpd_register_uri_handler(my_server, &reboot);
