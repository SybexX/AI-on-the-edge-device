...
esp_err_t get_data_file_handler(httpd_req_t *req)
{
    ...
    ESP_LOGD(TAG, "Search data files in /sdcard/log/data");
...
esp_err_t get_tflite_file_handler(httpd_req_t *req)
{
    ...
    ESP_LOGD(TAG, "Search TFLITE in /sdcard/config/");
...
esp_err_t get_config_file_handler(httpd_req_t *req)
{
    ...
    const char verz_name[] = "/sdcard/config";
    ESP_LOGD(TAG, "Search Configs in /sdcard/config/");
...
static esp_err_t upload_post_handler(httpd_req_t *req)
{
    ...
    if (filename[strlen(filename) - 1] == '/')
    {
        LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "Invalid filename: " + std::string(filename));
    ...
    {
        LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "File too large: " + std::to_string(req->content_len) + " bytes");
        // Respond with 400 Bad Request
    ...
    if (!pfile)
    {
        LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "Failed to create file: " + std::string(filepath));
        // Respond with 500 Internal Server Error
...
static esp_err_t delete_post_handler(httpd_req_t *req)
{
    ...
    if (httpd_req_get_url_query_str(req, _query, 200) == ESP_OK)
    {
        ...
        {
            LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "task is found: " + std::string(_valuechar));
    ...
    if (_task.compare("deldircontent") == 0)
    {
        ...
        delete_all_in_directory(zw); // Deletes all files and folders in a folder
        // delete_all_file_in_directory(zw); // Only deletes all files in one folder
    ...
    else
    {
        ...
        {
            LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "Failed to delete protected file : " + std::string(filename));
        ...
        if (stat(filepath, &file_stat) == -1)
        {
            ...
            LogFile.WriteToFile(ESP_LOG_INFO, TAG, "File does not exist: " + std::string(filename));
        }
