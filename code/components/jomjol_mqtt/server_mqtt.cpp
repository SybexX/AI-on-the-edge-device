...
bool sendHomeAssistantDiscoveryTopic(std::string group, std::string field, std::string name, std::string icon, std::string unit, std::string deviceClass, std::string stateClass, std::string entityCategory, int qos)
{
    ...
    payload = string("{") + "\"~\": \"" + maintopic + "\"," + "\"unique_id\": \"" + maintopic + "-" + configTopic + "\"," + "\"object_id\": \"" + maintopic + "_" + configTopic + "\"," + // This used to generate the Entity ID
    ...
    payload += "\"availability_topic\": \"~/" + std::string(LWT_TOPIC) + "\"," + "\"payload_available\": \"" + LWT_CONNECTED + "\"," + "\"payload_not_available\": \"" + LWT_DISCONNECTED + "\",";

    payload += string("\"device\": {") + "\"identifiers\": [\"" + maintopic + "\"]," + "\"name\": \"" + maintopic + "\"," + "\"model\": \"Meter Digitizer\"," + "\"manufacturer\": \"AI on the Edge Device\"," + "\"sw_version\": \"" + version + "\"," +
...
bool MQTThomeassistantDiscovery(int qos)
{
    ...
    for (int i = 0; i < (*NUMBERS).size(); ++i) {
        ...
        //                                                       Group   | Field                       | User Friendly Name                    | Icon                       | Unit                 | Device Class     | State Class       | Entity Category |
        //                                                       QoS
        allSendsSuccessed |= sendHomeAssistantDiscoveryTopic(group, "value", "Value", "gauge", valueUnit, meterType, value_state_class, "", qos); // State Class = "total_increasing" if <NUMBERS>.AllowNegativeRates = false, "measurement" in case of a
                                                                                                                                                  // thermometer, else use "total".
        ...
        // allSendsSuccessed |= sendHomeAssistantDiscoveryTopic(group,   "rate",                       "Rate (Unit/Minute)",                   "swap-vertical",             "",                    "",                "",                 "", qos); // Legacy,
        // always Unit per Minute
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG,
                        "Int. Heap Usage before Publishing Homeassistand Discovery Topics: " + to_string(aFreeInternalHeapSizeBefore) + ", after: " + to_string(aFreeInternalHeapSizeAfter) +
                            ", delta: " + to_string(aFreeInternalHeapSizeBefore - aFreeInternalHeapSizeAfter) + ", lowest free: " + to_string(aMinFreeInternalHeapSize));
...
bool publishSystemData(int qos)
{
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG,
                        "Int. Heap Usage before publishing System Topics: " + to_string(aFreeInternalHeapSizeBefore) + ", after: " + to_string(aFreeInternalHeapSizeAfter) +
                            ", delta: " + to_string(aFreeInternalHeapSizeBefore - aFreeInternalHeapSizeAfter) + ", lowest free: " + to_string(aMinFreeInternalHeapSize));
...
bool publishStaticData(int qos)
{
    ...
    allSendsSuccessed |= MQTTPublish(maintopic + "/" + "hostname", wifi_config.hostname, qos, retainFlag);
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG,
                        "Int. Heap Usage before Publishing Static Topics: " + to_string(aFreeInternalHeapSizeBefore) + ", after: " + to_string(aFreeInternalHeapSizeAfter) +
                            ", delta: " + to_string(aFreeInternalHeapSizeBefore - aFreeInternalHeapSizeAfter) + ", lowest free: " + to_string(aMinFreeInternalHeapSize));
