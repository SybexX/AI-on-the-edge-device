...
static esp_err_t scheduleSendingDiscovery_and_static_Topics_handler(httpd_req_t *req)
{
...
esp_err_t register_server_mqtt_uri(httpd_handle_t my_server)
{
    httpd_uri_t mqtt_publish = {
        .uri = "/mqtt_publish_discovery",
        .method = HTTP_GET,
        .handler = APPLY_BASIC_AUTH_FILTER(scheduleSendingDiscovery_and_static_Topics_handler),
        .user_ctx = rest_context // Pass server data as context
    };
    esp_err_t ret = httpd_register_uri_handler(my_server, &mqtt_publish);
