                    ...
                    int _keepalive, bool _SetRetainFlag, void *_callbackOnConnected)
{
    ...
    {
        std::ifstream cacert_ifs(_cacertfilename);
        if (cacert_ifs.is_open())
        {
            std::string cacert_content((std::istreambuf_iterator<char>(cacert_ifs)), (std::istreambuf_iterator<char>()));
            caCert = cacert_content;
            cacert_ifs.close();
