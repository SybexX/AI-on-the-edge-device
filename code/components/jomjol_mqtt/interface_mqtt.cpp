...
int MQTT_Init()
{
    ...
    if (client)
    {
        ...
        if (ret != ESP_OK)
        {
            LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "Could not register event (ret=" + std::string(esp_err_to_name(ret)) + ")!");
        ...
        if (ret != ESP_OK)
        {
            LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "Client start failed (retval=" + std::string(esp_err_to_name(ret)) + ")!");
            mqtt_initialized = false;
            return -1;
        }
        else
