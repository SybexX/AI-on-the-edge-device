...
bool WebhookPublish(std::vector<NumberPost *> *numbers)
{
    ...
    for (int i = 0; i < (*numbers).size(); ++i)
    {
        ...
        time_temp = std::string(buffer);

        cJSON *json = cJSON_CreateObject();
        cJSON_AddStringToObject(json, "timestamp", time_temp.c_str());
    ...
    esp_err_t ret = ESP_ERROR_CHECK_WITHOUT_ABORT(esp_http_client_perform(http_client));
...
void WebhookUploadPic(ImageData *Img)
{
    ...
    esp_err_t ret = ESP_ERROR_CHECK_WITHOUT_ABORT(esp_http_client_set_post_field(http_client, (const char *)Img->data, Img->size));

    ret = ESP_ERROR_CHECK_WITHOUT_ABORT(esp_http_client_perform(http_client));
