...
static void eth_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    else if (event_base == IP_EVENT && event_id == IP_EVENT_ETH_GOT_IP)
    {
        ...
        setStatusLED(WLAN_CONN, 0, true); // e.g., 0 means "ok"
    }
}
...
esp_err_t eth_init_W5500(void)
{
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to init spi bus, error=0x%x", ret);
        return ret;
    }
    ...
    ret = esp_eth_driver_install(&eth_config, &my_w5500_handle);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to install Ethernet driver, error=0x%x", ret);
        return ret;
    }
    ...
    uint8_t local_mac[6];
    ret = esp_derive_local_mac(local_mac, base_mac_addr);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to derive local MAC address from universal MAC address, error=0x%x", ret);
        return ret;
    }

    ret = esp_eth_ioctl(my_w5500_handle, ETH_CMD_S_MAC_ADDR, local_mac);
    if (ret != ESP_OK)
    {
    ...
    if (!network_config.ipaddress.empty() && !network_config.gateway.empty() && !network_config.netmask.empty())
    {
        ...
        ret = esp_netif_set_dns_info(my_w5500_netif, ESP_NETIF_DNS_MAIN, &dns_info);
        if (ret != ESP_OK)
        {
    ...
    ret = esp_netif_attach(my_w5500_netif, my_w5500_netif_glue);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to attaches esp_netif instance to the io driver handle, error=0x%x", ret);
        return ret;
    }

    ret = esp_eth_start(my_w5500_handle);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to start Ethernet driver, error=0x%x", ret);
        return ret;
    }
...
void eth_deinit_W5500(void)
{
    ...
    ret = esp_eth_driver_uninstall(my_w5500_handle);
    if (ret != ESP_OK)
    {
