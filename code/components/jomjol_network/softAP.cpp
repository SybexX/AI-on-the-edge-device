...
void SendHTTPResponse(httpd_req_t *req)
{
    ...
    message += "This mode is always started if one of the following files is missing: /network.ini or the /config/config.ini.<p>";
    message += "The setup is done in 3 steps: 1. upload full inital configuration (sd-card content), 2. store WIFI access information, 3. reboot (and connect to WIFIs)<p><p>";
    message += "Please follow the below instructions.<p>";
    httpd_resp_send_chunk(req, message.c_str(), strlen(message.c_str()));

    isWifiINI = FileExists(NETWORK_CONFIG_FILE);
    ...
    if (!isWifiINI) {
        message = "<h3>2. WIFI access credentials</h3><p>";
        message += "<table>";
        message += "<tr><td>WIFI-SSID</td><td><input type=\"text\" name=\"ssid\" id=\"ssid\"></td><td>SSID of the WIFI</td></tr>";
        message += "<tr><td>WIFI-Password</td><td><input type=\"text\" name=\"password\" id=\"password\"></td><td>ATTENTION: the password will not be encrypted during the sending.</td><tr>";
        message += "</table><p>";
        message += "<h4>ATTENTION:<h4>Be sure about the WIFI settings. They cannot be reset afterwards. If ssid or password is wrong, you need to take out the sd-card and manually change them in \"network.ini\"!<p>";
        httpd_resp_send_chunk(req, message.c_str(), strlen(message.c_str()));
        ...
        //        message += "<tr><td>RSSI Threshold</td><td><input type=\"number\" name=\"name\" id=\"threshold\" min=\"-100\"  max=\"0\" step=\"1\" value = \"0\"></td><td>WIFI Mesh Parameter: Threshold for RSSI value to check for start switching access
        //        point in a mesh system (if actual RSSI is lower). Possible values: -100 to 0, 0 = disabled - Value will be transfered to network.ini at next startup)</td></tr>"; httpd_resp_send_chunk(req, message.c_str(), strlen(message.c_str()));


        message = "<button class=\"button\" type=\"button\" onclick=\"wr()\">Write network.ini</button>";
    ...
    message += "After triggering the reboot, the zip-files gets extracted and written to the sd-card.<br>The ESP32 will restart two times and then connect to your access point. Please find the IP in your router settings and access it with the new "
               "ip-address.<p>";
    message += "The first update and initialization process can take up to 3 minutes before you find it in the wifi. Error logs can be found on the console / serial logout.<p>Have fun!<p>";
...
esp_err_t config_ini_handler(httpd_req_t *req)
{
    ...
    text = ";++++++++++++++++++++++++++++++++++\n";
    text += "; AI on the edge - WIFI configuration\n";
    text += "; ssid: Name of WIFI network (mandatory), e.g. \"WIFI-SSID\"\n";
    text += "; password: Password of WIFI network (mandatory), e.g. \"PASSWORD\"\n\n";
...
void CheckStartAPMode()
{
    ...
    if (!isWifiINI) {
        ESP_LOGW(TAG, "network.ini not found!");
    }

    if (!isConfigINI || !isWifiINI) {
