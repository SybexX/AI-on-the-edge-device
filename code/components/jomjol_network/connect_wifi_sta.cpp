...
esp_err_t wifi_init_sta(void)
{
	...
	my_netif_sta = esp_netif_create_default_wifi_sta();
	...
	if (!network_config.ipaddress.empty() && !network_config.gateway.empty() && !network_config.netmask.empty())
	{
		...
		esp_netif_dhcpc_stop(my_netif_sta); // Stop DHCP service
		...
		esp_netif_set_ip_info(my_netif_sta, &ip_info); // Set static IP configuration
	}
	else
	{
	...
	if (!network_config.ipaddress.empty() && !network_config.gateway.empty() && !network_config.netmask.empty())
	{
		...
		ret = esp_netif_set_dns_info(my_netif_sta, ESP_NETIF_DNS_MAIN, &dns_info);
		if (ret != ESP_OK)
		{
			LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "esp_netif_set_dns_info: Error: " + std::string(esp_err_to_name(ret)));
			return ret;
		}
	}
	...
	if (ret != ESP_OK)
	{
		LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "esp_event_handler_instance_register - GOT_IP: Error: " + std::string(esp_err_to_name(ret)));
		return ret;
	}

#ifdef WLAN_USE_MESH_ROAMING
	// Register an instance of event handler to the default loop.
	ret = esp_event_handler_instance_register(WIFI_EVENT, WIFI_EVENT_STA_BSS_RSSI_LOW, &esp_bss_rssi_low_handler, NULL, NULL);
	if (ret != ESP_OK)
	{
		LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "esp_event_handler_instance_register - BSS_RSSI_LOW: Error: " + std::string(esp_err_to_name(ret)));
		return ret;
	}
#endif
...
// wifi_config.sta.ft_enabled = 1;	 // 802.11r (BSS Fast Transition) -> Upcoming IDF version 5.0 will support 11r
	...
	if (ret != ESP_OK)
	{
		...
		{
			LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "esp_wifi_set_config: SSID password invalid! Error: " + std::string(esp_err_to_name(ret)));
		}
		else
		{
	...
	{
		ret = esp_netif_set_hostname(my_netif_sta, network_config.hostname.c_str());
		if (ret != ESP_OK)
		{
			LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "Failed to set hostname! Error: " + std::string(esp_err_to_name(ret)));
		}
		else
		{
		...
		if (ret != ESP_OK)
		{
			LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "mdns_init failed! Error: " + std::string(esp_err_to_name(ret)));
		}
		else
		{
			// set mdns hostname
...
void wifi_deinit_sta()
{
	...
	ESP_LOGD(TAG, "esp_netif_destroy(my_netif_sta)");
	esp_netif_destroy(my_netif_sta);
