...
{
    ImageData *image_temp = (ImageData *)context;
    uint8_t *voidstart = image_temp->data;
    uint8_t *datastart = (uint8_t *)data;

    if ((image_temp->size < MAX_JPG_SIZE))
    {
        // Abort copy to prevent buffer overflow
        voidstart += image_temp->size;
...
{
    ImageData *image_temp = new ImageData;

    RGBImageLock();
    stbi_write_jpg_to_func(writejpghelp, image_temp, width, height, channels, rgb_image, quality);
    RGBImageRelease();
...
void CImageBasis::drawRect(int x, int y, int dx, int dy, int r, int g, int b, int thickness)
{
    ...
    int _x1 = x - thickness + 1;
    int _x2 = x + dx + thickness - 1;
    int _y1 = y;
    int _y2 = y;
...
void CImageBasis::drawLine(int x1, int y1, int x2, int y2, int r, int g, int b, int thickness)
{
    ...
    for (int _thick = 0; _thick <= thickness; ++_thick)
    {
        for (int _x = x1 - _thick; _x <= x2 + _thick; ++_x)
        {
            ...
            {
                _y1 = (y2 - y1) * (float)(_x - x1) / (float)(x2 - x1) + y1;
                _y2 = (y2 - y1) * (float)(_x + 1 - x1) / (float)(x2 - x1) + y1;
            }

            for (int _y = _y1 - _thick; _y <= _y2 + _thick; _y++)
            {
                if (isInImage(_x, _y))
                {
...
void CImageBasis::LoadFromMemory(stbi_uc *_buffer, int len)
{
    ...
    if ((width * height * channels) == 0)
    {
        LogFile.WriteToFile(ESP_LOG_ERROR, TAG,
                            "Image with size 0 loaded --> reboot to be done! "
...
#ifdef DEBUG_DETAIL_ON
    std::string image_temp = "CImageBasis after load " + _image;
    ESP_LOGD(TAG, "%s", image_temp.c_str());
...
void CImageBasis::setImageNegative(void)
{
    RGBImageLock();
...
void CImageBasis::setImageContrast(float _contrast) // input range [-100..100]
{
    stbi_uc *p_source;
