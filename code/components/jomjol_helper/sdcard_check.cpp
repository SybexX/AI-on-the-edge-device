...
int SDCardCheckRW(void)
{
    ...
    ESP_LOGI(TAG, "Basic R/W check started...");
    ...
    if (pFile == NULL)
    {
        ...
        ESP_LOGE(TAG, "Basic R/W check: (E1) No able to open file to write");
        return -1;
    }
    else
    {
        ...
        if (fwrite(sMessage.c_str(), sMessage.length(), 1, pFile) == 0)
        {
            ...
            ESP_LOGE(TAG, "Basic R/W check: (E2) Not able to write file");
            fclose(pFile);
    ...
    if (pFile == NULL)
    {
        ...
        ESP_LOGE(TAG, "Basic R/W check: (E3) Not able to open file to read back");
        unlink("/sdcard/sdcheck.txt");
        return -3;
    }
    else
    {
        ...
        if (fgets(cReadBuf, sizeof(cReadBuf), pFile) == 0)
        {
            ...
            ESP_LOGE(TAG, "Basic R/W check: (E4) Not able to read file back");
            fclose(pFile);
            unlink("/sdcard/sdcheck.txt");
            return -4;
        }
        else
        {
            if (esp_rom_crc16_le(0, (uint8_t *)cReadBuf, strlen(cReadBuf)) != iCRCMessage)
            {
                ...
                ESP_LOGE(TAG, "Basic R/W check: (E5) Read back, but wrong CRC");
                fclose(pFile);
                unlink("/sdcard/sdcheck.txt");
                return -5;
            }
    ...
    if (unlink("/sdcard/sdcheck.txt") != 0)
    {
        ...
        ESP_LOGE(TAG, "Basic R/W check: (E6) Unable to delete the file");
        return -6;
    }

    // LogFile.WriteToFile(ESP_LOG_INFO, TAG, "Basic R/W check successful");
    ESP_LOGI(TAG, "Basic R/W check successful");
    return 0;
}
...
bool SDCardCheckFolderFilePresence()
{
    ...
    ESP_LOGI(TAG, "Folder/file presence check started...");
    ...
    if (stat("/sdcard/config", &sb) != 0)
    {
        ...
        ESP_LOGE(TAG, "Folder/file check: Folder /config not found");
        bRetval = false;
    }
    ...
    if (stat("/sdcard/html", &sb) != 0)
    {
        ...
        ESP_LOGE(TAG, "Folder/file check: Folder /html not found");
        bRetval = false;
    }
    ...
    if (stat("/sdcard/firmware", &sb) != 0)
    {
        ...
        ESP_LOGE(TAG, "Folder/file check: Folder /firmware not found");
        bRetval = false;
    }
    ...
    if (stat("/sdcard/img_tmp", &sb) != 0)
    {
        ...
        ESP_LOGE(TAG, "Folder/file check: Folder /img_tmp not found");
        bRetval = false;
    }
    ...
    if (stat("/sdcard/log", &sb) != 0)
    {
        ...
        ESP_LOGE(TAG, "Folder/file check: Folder /log not found");
        bRetval = false;
    }
    ...
    if (stat("/sdcard/demo", &sb) != 0)
    {
        ...
        ESP_LOGE(TAG, "Folder/file check: Folder /demo not found");
        bRetval = false;
    }
    ...
    if (stat("/sdcard/wlan.ini", &sb) != 0)
    {
        ...
        ESP_LOGE(TAG, "Folder/file check: File /wlan.ini not found");
        bRetval = false;
    }
    ...
    if (stat("/sdcard/config/config.ini", &sb) != 0)
    {
        ...
        ESP_LOGE(TAG, "Folder/file check: File /config/config.ini not found");
        bRetval = false;
    }
    ...
    if (stat("/sdcard/html/index.html", &sb) != 0)
    {
        ...
        ESP_LOGE(TAG, "Folder/file check: File /html/index.html not found");
        bRetval = false;
    }
    ...
    if (stat("/sdcard/html/ota_page.html", &sb) != 0)
    {
        ...
        ESP_LOGE(TAG, "Folder/file check: File /html/ota.html not found");
        bRetval = false;
    }
    ...
    if (stat("/sdcard/html/log.html", &sb) != 0)
    {
        ...
        ESP_LOGE(TAG, "Folder/file check: File /html/log.html not found");
        bRetval = false;
    }
    ...
    if (stat("/sdcard/html/common.js", &sb) != 0)
    {
        ...
        ESP_LOGE(TAG, "Folder/file check: File /html/common.js not found");
        bRetval = false;
    }
    ...
    if (stat("/sdcard/html/version.txt", &sb) != 0)
    {
        ...
        ESP_LOGE(TAG, "Folder/file check: File /html/version.txt not found");
        bRetval = false;
    }
    ...
    if (bRetval)
    {
        ...
        ESP_LOGI(TAG, "Folder/file presence check successful");
    }
