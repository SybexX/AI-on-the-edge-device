...
void GpioPin::init()
{
    ...
    //    if (_interruptType != GPIO_INTR_DISABLE) {                // ohne GPIO_PIN_MODE_FLASH_WS281X, wenn das genutzt wird, dann soll
    //    auch der Handler hier nicht initialisiert werden, da das dann über SmartLED erfolgt.
    if ((_interruptType != GPIO_INTR_DISABLE) && (_interruptType != GPIO_PIN_MODE_FLASH_WS281X)) {
        // hook isr handler for specific gpio pin
...
void GpioPin::setValue(bool value, gpio_set_source setSource, std::string *errorText)
{
    ...
    if ((_mode != GPIO_PIN_MODE_OUTPUT) && (_mode != GPIO_PIN_MODE_OUTPUT_PWM) && (_mode != GPIO_PIN_MODE_FLASH_PWM)) {
...
void GpioHandler::flashLightEnable(bool value)
{
    ...
    if (gpioMap != NULL) {
        for (std::map<gpio_num_t, GpioPin *>::iterator it = gpioMap->begin(); it != gpioMap->end(); ++it) {
            if (it->second->getMode() == GPIO_PIN_MODE_FLASH_PWM) {
                std::string resp_str = "";
            ...
            else if (it->second->getMode() == GPIO_PIN_MODE_FLASH_WS281X) {
...
#elif defined(BOARD_WROVER_KIT_ESP32)
    switch (pinNr) {
        case 1:
            return GPIO_NUM_12;
        case 2:
...
gpio_pin_mode_t GpioHandler::resolvePinMode(std::string input)
{
    ...
    if (input == "flash-pwm") {
        return GPIO_PIN_MODE_FLASH_PWM;
    }
    if (input == "flash-ws281x") {
        return GPIO_PIN_MODE_FLASH_WS281X;
    }
