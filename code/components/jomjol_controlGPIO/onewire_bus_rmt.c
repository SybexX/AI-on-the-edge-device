...
static bool onewire_rmt_check_presence_pulse(rmt_symbol_word_t *rmt_symbols, size_t symbol_num)
{
    if (symbol_num >= 2)
    {
        ...
        {
            // bus is low before reset pulse(first pulse after rmt channel init)
...
static void onewire_rmt_decode_data(rmt_symbol_word_t *rmt_symbols, size_t symbol_num, uint8_t *decoded_bytes)
{
    ...
    {
        if (rmt_symbols[i].duration0 > ONEWIRE_SLOT_BIT_SAMPLE_TIME)
        {
            // 0 bit
...
esp_err_t onewire_new_bus_rmt(onewire_rmt_config_t *config, onewire_bus_handle_t *handle_out)
{
    ...
    rmt_bytes_encoder_config_t bytes_encoder_config = {
        ...
        .flags.msb_first = 0};
    ESP_GOTO_ON_ERROR(rmt_new_bytes_encoder(&bytes_encoder_config, &handle->tx_bytes_encoder), err, TAG, "create data tx encoder failed");

    // create rmt copy encoder to transmit 1-wire reset pulse or bits
    rmt_copy_encoder_config_t copy_encoder_config = {};
    ESP_GOTO_ON_ERROR(rmt_new_copy_encoder(&copy_encoder_config, &handle->tx_copy_encoder), err, TAG, "create reset pulse tx encoder failed");
    ...
    ESP_GOTO_ON_ERROR(rmt_new_rx_channel(&onewire_rx_channel_cfg, &handle->rx_channel), err, TAG, "create rmt rx channel failed");
    ...
    ESP_GOTO_ON_ERROR(rmt_new_tx_channel(&onewire_tx_channel_cfg, &handle->tx_channel), err, TAG, "create rmt tx channel failed");
    ...
    ESP_GOTO_ON_ERROR(rmt_rx_register_event_callbacks(handle->rx_channel, &cbs, handle), err, TAG, "enable rmt rx channel failed");
...
esp_err_t onewire_bus_read_bytes(onewire_bus_handle_t handle, uint8_t *rx_data, size_t rx_data_size)
{
    ...
    ESP_RETURN_ON_FALSE(!(rx_data_size > handle->max_rx_bytes), ESP_ERR_INVALID_ARG, TAG, "rx_data_size too large for buffer to hold");
