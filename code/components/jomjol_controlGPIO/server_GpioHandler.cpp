...
GpioHandler::GpioHandler(std::string configFile, httpd_handle_t my_server)
{
    ESP_LOGI(TAG, "start GpioHandler");
    _configFile = configFile;
    _httpServer = my_server;

    ESP_LOGI(TAG, "register GPIO Uri");
    esp_err_t ret = register_server_gpio_uri(my_server);
    if (ret != ESP_OK)
    {
        LogFile.WriteToFile(ESP_LOG_ERROR, TAG, "register_server_gpio_uri: Error: " + std::string(esp_err_to_name(ret)));
    }
}
...
static esp_err_t call_http_request_handler(httpd_req_t *req)
{
    ESP_LOGD(TAG, "call_http_request_handler");
...
esp_err_t GpioHandler::register_server_gpio_uri(httpd_handle_t my_server)
{
    httpd_uri_t gpio = {
        .uri = "/GPIO",
        .method = HTTP_GET,
        .handler = APPLY_BASIC_AUTH_FILTER(call_http_request_handler),
        .user_ctx = rest_context // Pass server data as context
    };
    esp_err_t ret = httpd_register_uri_handler(my_server, &gpio);
...
void gpio_handler_create(httpd_handle_t my_server)
{
    if (gpioHandler == NULL)
    {
        gpioHandler = new GpioHandler(CONFIG_FILE, my_server);
    }
}
